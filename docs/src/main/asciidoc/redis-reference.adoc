...
Adding the dependency is the only switch: the extension builds on `quarkus-redis-client`, so the
`quarkus.redis.*` configuration, the Dev Services and the health check are shared and work unchanged.
...
The Vert.x Redis client beans remain available and keep backing the health check and `quarkus-redis-cache`.
