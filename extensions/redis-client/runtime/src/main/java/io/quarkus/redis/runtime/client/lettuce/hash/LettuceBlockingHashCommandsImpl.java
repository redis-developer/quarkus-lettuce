...
/**
 * Blocking wrapper around {@link LettuceReactiveHashCommandsImpl}.
 * <p>
 * Each method awaits the reactive result for the configured {@code timeout}. Must not be invoked from
 * an event loop thread. The {@code hscan} methods return immediately; the returned cursor awaits each
 * page instead.
 *
 ...
 */
public class LettuceBlockingHashCommandsImpl<K, F, V> implements HashCommands<K, F, V> {

    private final RedisDataSource dataSource;
    private final ReactiveHashCommands<K, F, V> reactive;
    private final Duration timeout;

    public LettuceBlockingHashCommandsImpl(RedisDataSource dataSource, ReactiveHashCommands<K, F, V> reactive,
            Duration timeout) {
    ...
    @SafeVarargs
    @Override
    public final int hdel(K key, F... fields) {
    ...
    @SafeVarargs
    @Override
    public final Map<F, V> hmget(K key, F... fields) {
