...
/**
 * Blocking wrapper around {@link LettuceReactiveKeyCommandsImpl}.
 * <p>
 ...
 */
public class LettuceBlockingKeyCommandsImpl<K> implements KeyCommands<K> {

    private final RedisDataSource dataSource;
    private final ReactiveKeyCommands<K> reactive;
    private final Duration timeout;

    public LettuceBlockingKeyCommandsImpl(RedisDataSource dataSource, ReactiveKeyCommands<K> reactive,
            Duration timeout) {
    ...
    @SafeVarargs
    @Override
    public final int exists(K... keys) {
    ...
    @Override
    public boolean pexpire(K key, Duration duration, ExpireArgs expireArgs) {
        return reactive.pexpire(key, duration, expireArgs).await().atMost(timeout);
    }
    ...
    public boolean pexpire(K key, long milliseconds, ExpireArgs expireArgs) {
        return reactive.pexpire(key, milliseconds, expireArgs).await().atMost(timeout);
    }
    ...
    public boolean pexpireat(K key, long timestamp, ExpireArgs expireArgs) {
        return reactive.pexpireat(key, timestamp, expireArgs).await().atMost(timeout);
    }
    ...
    @SafeVarargs
    @Override
    public final int unlink(K... keys) {
