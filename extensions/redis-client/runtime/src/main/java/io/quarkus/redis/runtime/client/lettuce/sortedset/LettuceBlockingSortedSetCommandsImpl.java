...
/**
 * Blocking wrapper around {@link LettuceReactiveSortedSetCommandsImpl}.
 * <p>
 * Each method awaits the reactive result for the configured {@code timeout}. Must not be invoked from
 * an event loop thread. The {@code zscan} methods return immediately; the returned cursor awaits each
 * page instead. The blocking API narrows a few nullable results to {@code Optional*}, which is the only
 * conversion happening here.
 *
 * @param <K> the key type
 * @param <V> the type of the scored member
 */
public class LettuceBlockingSortedSetCommandsImpl<K, V> implements SortedSetCommands<K, V> {

    private final RedisDataSource dataSource;
    private final ReactiveSortedSetCommands<K, V> reactive;
    private final Duration timeout;

    public LettuceBlockingSortedSetCommandsImpl(RedisDataSource dataSource, ReactiveSortedSetCommands<K, V> reactive,
            Duration timeout) {
    ...
    public OptionalDouble zaddincr(K key, ZAddArgs zAddArgs, double score, V member) {
        return reactive.zaddincr(key, zAddArgs, score, member)
                .map(LettuceBlockingSortedSetCommandsImpl::toOptionalDouble)
    ...
    @SafeVarargs
    @Override
    public final long zdiffstore(K destination, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final List<V> zinter(ZAggregateArgs arguments, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> zinterWithScores(ZAggregateArgs arguments, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final long zinterstore(K destination, ZAggregateArgs arguments, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final ScoredValue<V> zmpopMin(K... keys) {
        return reactive.zmpopMin(keys).await().atMost(timeout);
    }

    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> zmpopMin(int count, K... keys) {
        return reactive.zmpopMin(count, keys).await().atMost(timeout);
    }

    @SafeVarargs
    @Override
    public final ScoredValue<V> zmpopMax(K... keys) {
        return reactive.zmpopMax(keys).await().atMost(timeout);
    }

    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> zmpopMax(int count, K... keys) {
        return reactive.zmpopMax(count, keys).await().atMost(timeout);
    }

    @SafeVarargs
    @Override
    public final ScoredValue<V> bzmpopMin(Duration timeout, K... keys) {
        return reactive.bzmpopMin(timeout, keys).await().atMost(this.timeout);
    }

    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> bzmpopMin(Duration timeout, int count, K... keys) {
        return reactive.bzmpopMin(timeout, count, keys).await().atMost(this.timeout);
    }

    @SafeVarargs
    @Override
    public final ScoredValue<V> bzmpopMax(Duration timeout, K... keys) {
        return reactive.bzmpopMax(timeout, keys).await().atMost(this.timeout);
    }

    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> bzmpopMax(Duration timeout, int count, K... keys) {
        return reactive.bzmpopMax(timeout, count, keys).await().atMost(this.timeout);
    }

    @SafeVarargs
    @Override
    public final List<OptionalDouble> zmscore(K key, V... members) {
        return reactive.zmscore(key, members)
                .map(list -> list.stream().map(LettuceBlockingSortedSetCommandsImpl::toOptionalDouble)
    ...
    @SafeVarargs
    @Override
    public final KeyValue<K, ScoredValue<V>> bzpopmin(Duration timeout, K... keys) {
        return reactive.bzpopmin(timeout, keys).await().atMost(this.timeout);
    }

    @SafeVarargs
    @Override
    public final KeyValue<K, ScoredValue<V>> bzpopmax(Duration timeout, K... keys) {
    ...
    public OptionalLong zrank(K key, V member) {
        return reactive.zrank(key, member)
                .map(LettuceBlockingSortedSetCommandsImpl::toOptionalLong)
                .await().atMost(timeout);
    }

    @SafeVarargs
    @Override
    public final int zrem(K key, V... members) {
    ...
    @SafeVarargs
    @Override
    public final List<V> zunion(ZAggregateArgs args, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final List<ScoredValue<V>> zunionWithScores(ZAggregateArgs args, K... keys) {
    ...
    @SafeVarargs
    @Override
    public final long zunionstore(K destination, ZAggregateArgs args, K... keys) {
